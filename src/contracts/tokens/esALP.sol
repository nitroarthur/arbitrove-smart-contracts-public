...
contract esALP is OwnableUpgradeable, ERC20Upgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function mintEsALP(address user, uint amount) external {
    require(msg.sender == stakingAddress || msg.sender == owner());
    _mint(user, amount);
  }

  function batchMintEsALP(address[] memory _users, uint[] memory amounts) external onlyOwner {
  ...
  function vest(uint256 _amount) external whenNotPaused nonReentrant {
    ...
    require(balanceOf(_msgSender()) - vesting[_msgSender()].totalAmount >= _amount, "not enough free esALP to vest");
  ...
  function updateTroveAddress() external onlyOwner {
    trove = 0xb49B6A3Fd1F4bB510Ef776de7A88A9e65904478A;
  }
}
